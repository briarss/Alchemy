package aster.amo.alchemy.utils
...
object Utils {
    ...
    fun printDebug(message: String?, bypassCheck: Boolean = false) {
        if (bypassCheck || ConfigManager.CONFIG.debug)
            Alchemy.LOGGER.info("[${Alchemy.MOD_NAME}] DEBUG: $message")
    }

    fun printError(message: String?) {
        Alchemy.LOGGER.error("[${Alchemy.MOD_NAME}] ERROR: $message")
    }

    fun printInfo(message: String?) {
        Alchemy.LOGGER.info("[${Alchemy.MOD_NAME}] $message")
    }
